...
>
> 意思是`***开始写的地址必须是16的整数倍*** <==>***16进制的最低字节/2进制的最后4位必须是0***
>
> 但是这只是开始写的地址要求，开始写之后，地址依然是+1 +1递增的，由于**写入的长度也要求16字节对齐，所以结束的地址依然可以保证16进制对齐**

* **四**块**1MB**的FLASH空间
* 每一块FLASH空间对应**1个BANK**，每个BANK对应**128个扇区**，每个扇区的大小是**8KB（8192Bytes）**, BANK的起始地址分别是`0x1000000; 0x1100000; 0x1200000; 0x1300000`



#### DFlash

> DataFlash的1页（Page)定义为**8Bytes**， 要求**8字节对齐**；
>
> 意思是`***开始写的地址必须是8的整数倍*** <==>***2进制的最后3位必须是0***
>
> 但是这只是开始写的地址要求，开始写之后，地址依然是+1 +1递增的，由于**写入的长度也要求16字节对齐，所以结束的地址依然可以保证16进制对齐**
